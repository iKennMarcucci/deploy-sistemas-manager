...
const VerAplazamiento = () => {
    ...
    useEffect(() => {
        fetchAplazamientoData();
    }, [id, backendUrl]);

    const fetchAplazamientoData = () => {
        setLoading(true);
        fetch(`${backendUrl}/solicitud/${id}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('No se pudo cargar la información del aplazamiento');
                }
            ...
            .catch(err => {
                ...
                setLoading(false);
                showAlerta('No se pudo cargar la información del aplazamiento', 'error', 'Error de conexión');
            });
    };
    ...
    const handleApproveAplazamiento = async () => {
        if (!selectedFile) {
        ...
        try {
            ...
            showAlerta(data.mensaje || 'Aplazamiento aprobado con éxito', 'success', 'Solicitud aprobada');
        } catch (err) {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                ...
                <Boton onClick={() => navigate('/aplazamientos')} startContent={<ArrowLeft size={18} />}>
                    Volver a Aplazamientos
                </Boton>
            </div>
        );
    }
        ...
        <div className='flex flex-col w-full items-center p-4'>
            <div className='w-full flex flex-row justify-between mb-6'>
                <button
                    ...
                    onClick={() => navigate('/aplazamiento')}
                >
                    <ArrowLeft />
                </button>
                <p className='text-center text-titulos'>Detalles de Solicitud de Aplazamiento</p>
                <div className='w-[40px]'></div>
            </div>

            {/* Información del aplazamiento */}
            <p className='text-normal'>Información del aplazamiento</p>
            <Divider className='mb-4' />

            <div className='w-full flex flex-col'>
                {/* Primera fila: Estudiante y Semestre */}
            ...
            {/* Informe del aplazamiento (solo se muestra si está aprobado) */}
            {solicitud.estaAprobado && solicitud.soporte && (
                <>
                    <p className='text-normal mt-8'>Informe de Aplazamiento de Semestre</p>
                    <Divider className='mb-4' />
                    <div className='w-full px-4'>
            ...
            <Modal
                ...
                footer={
                    <div className="flex justify-end w-full">
                        <Boton
                            color="success"
                            onClick={handleApproveAplazamiento}
...
export default VerAplazamiento;