...
const Cancelaciones = () => {
    ...
    const fetchSolicitudes = () => {
        fetch(`${backendUrl}/cancelacion`)
            ...
            .catch((error) => {
                showAlerta("Error al cargar las solicitudes de cancelación", "error", "Error de conexión");
            });
    };
    ...
    // Modificar la función handleEditSolicitud para que espere correctamente a que se carguen las materias
    const handleEditSolicitud = async (solicitud) => {
        ...
        try {
            ...
            if (solicitud.estudianteId) {
                ...
                const materiasDelEstudiante = await cargarMateriasPorEstudiante(solicitud.estudianteId);

                // Programamos la selección de la materia después de que las materias se carguen
                setTimeout(() => {
                    // Verificamos si la materia buscada está entre las cargadas
                    ...
                    if (materiaEncontrada) {
                        ...
                        setEditMateriaId(solicitud.grupoCohorteId?.toString() || '');
                    }
                }, 300); // Aumentamos el tiempo de espera para garantizar que se carguen las materias
            }

            // Abrir el modal de edición después de haber cargado los datos
            setIsEditModalOpen(true);
        } catch (error) {
            showAlerta("Error al preparar el formulario de edición", "error", "Error de conexión");
        }
    };
    ...
    const cargarMateriasPorEstudiante = async (estudianteId) => {
        try {
            const response = await fetch(`${backendUrl}/matriculas/estudiante/${estudianteId}`);
            if (!response.ok) {
                throw new Error("Error al cargar materias del estudiante");
            }
            const data = await response.json();
            // Solo considerar materias que estén en estado "En curso"
            const materiasEnCurso = data.filter(matricula => matricula.estadoMatriculaNombre === 'En curso');
            // Actualizar el estado
            setMaterias(materiasEnCurso);
            return materiasEnCurso;
        } catch (error) {
            showAlerta('Error al cargar las materias del estudiante', 'error', 'Error de conexión');
            setMaterias([]);
            return [];
        }
    };
    ...
    const handleEditEstudianteChange = async (id) => {
        ...
        // Resetear la materia cuando cambia el estudiante
        setEditMateriaId('');

        if (id) {
            try {
                // Cargar las materias del estudiante seleccionado
                await cargarMateriasPorEstudiante(id);

                // Si estamos en modo edición y el estudiante no ha cambiado, intentar mantener la materia seleccionada
                if (editSolicitudId && informacion) {
                    const solicitudActual = informacion.find(sol => sol.Id === editSolicitudId);
                    if (solicitudActual && solicitudActual.estudianteId.toString() === id) {
                        // Verificar si la materia anterior está en la nueva lista cargada
                        const materiaExiste = materias.some(mat =>
                            mat.id.toString() === solicitudActual.grupoCohorteId?.toString()
                        );

                        if (materiaExiste) {
                            setEditMateriaId(solicitudActual.grupoCohorteId?.toString() || '');
                        }
                    }
                }
            } catch (error) {
    ...
    const crearSolicitudCancelacion = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage = data.mensaje || data.message || data.error || 'Error al crear la solicitud de cancelación';
    ...
    const actualizarSolicitudCancelacion = async (e) => {
        ...
        try {
            const response = await fetch(`${backendUrl}/cancelacion/actualizar/${editSolicitudId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (!response.ok) {
                // Si la respuesta no es exitosa, extraemos el mensaje de error
                const errorMessage = data.mensaje || data.message || data.error || 'Error al actualizar la solicitud de cancelación';
                throw new Error(errorMessage);
            }
            ...
            fetchSolicitudes(); // Actualizar tabla de solicitudes
            showAlerta(data.mensaje || 'Solicitud de cancelación actualizada con éxito', 'success', 'Solicitud actualizada');
        } catch (error) {
            // Mostrar el mensaje de error en el AlertaModal en lugar de en el texto del formulario
            showAlerta(error.message, 'error', 'Error de validación');
            setEditFormError(''); // Limpiamos el editFormError ya que usamos AlertaModal
        } finally {
            setEditando(false);
        }
    };
    ...
    const submitAprobarCancelacion = async () => {
        ...
        try {
            const response = await fetch(`${backendUrl}/cancelacion/aprobar/${currentSolicitud.Id}`, {
                method: 'POST',
                headers: {
            ...
            setFileName("");
            showAlerta(data.mensaje || 'Solicitud de cancelación aprobada con éxito', 'success', 'Solicitud aprobada');
        } catch (err) {
            setFileError(err.message);
            // Añadir aquí una llamada a showAlerta
    ...
    return (
        <div className="w-full p-4">
            <p className="text-center text-titulos">Cancelación de Materias</p>
            ...
            <Modal
                ...
                cuerpo={
                    <div>
                        ...
                        {currentSolicitud && (
                            <p>
                                ¿Está seguro que desea aprobar la solicitud de cancelación de la materia <strong>{currentSolicitud.Materia}</strong> para el estudiante <strong>{currentSolicitud.Estudiante}</strong>?
                            </p>
                        )}
                        <p className="mt-2 mb-1">
                            Esta acción cancelará la materia {currentSolicitud?.Materia} del grupo {currentSolicitud?.Grupo} para el semestre {currentSolicitud?.Semestre}.
                        </p>
                    ...
                    >
                        ...
                        <div className="w-full py-4">
                            <Autocomplete
                                ...
                                defaultSelectedKey={editMateriaId}
                                label="Materia a Cancelar"
                                size="md"
                                placeholder="Selecciona la materia"
                                labelPlacement="outside"
