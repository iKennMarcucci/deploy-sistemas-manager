...
const EditarContraprestacion = () => {
    ...
    const [estudianteSeleccionado, setEstudianteSeleccionado] = useState('Cargando estudiante...')
    const [tipoSeleccionado, setTipoSeleccionado] = useState('Cargando tipo...')
    ...
    useEffect(() => {
        const fetchContraprestacion = async () => {
            ...
            try {
                ...
                const nombreCompleto = `${data.primerNombre || ''} ${data.segundoNombre ? data.segundoNombre + ' ' : ''}${data.primerApellido || ''} ${data.segundoApellido || ''}`.trim()
                setEstudianteCodigo(data.estudianteCodigo || '')
                setEstudianteSeleccionado(`${nombreCompleto}${data.estudianteCodigo ? ' - ' + data.estudianteCodigo : ''}`)

                // Establecer tipo de contraprestación seleccionado
                setTipoSeleccionado(data.tipoContraprestacionNombre || 'Tipo no especificado')
                ...
                showAlerta(
                    err.message || 'No se pudo cargar la información de la contraprestación',
                    'error',
                    'Error al cargar'
                )
                setLoadingData(false)
    ...
    const actualizarContraprestacion = async () => {
        ...
        try {
            const response = await fetch(`${backendUrl}/contraprestaciones/actualizar/${id}`, {
                method: 'PUT',
                headers: {
            ...
            if (response.ok) {
                ...
                showAlerta(
                    data.message || data.mensaje || "Contraprestación actualizada con éxito",
                    'success',
    ...
    if (loadingData) {
        return (
            <div className='flex flex-col w-full items-center p-4'>
                <p className="text-lg text-center">Cargando información de la contraprestación...</p>
            </div>
        )
    }
                                ...
                                onSelectionChange={(id) => {
                                    if (id) {
                                        ...
                                        setEstudianteSeleccionado(
                                            `${estudianteSeleccionado.nombre} ${estudianteSeleccionado.nombre2 || ''} ${estudianteSeleccionado.apellido} ${estudianteSeleccionado.apellido2 || ''} - ${estudianteSeleccionado.codigo}`
                                        );
                                        setEstudianteCodigo(estudianteSeleccionado.codigo || '');
                                    } else {
