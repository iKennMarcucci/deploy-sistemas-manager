...
const Contraprestaciones = () => {
  ...
  const [cargandoContraprestaciones, setCargandoContraprestaciones] =
    useState(true)
  ...
  const fetchContraprestaciones = () => {
    ...
    fetch(`${backendUrl}/contraprestaciones`)
      ...
      .finally(() => {
        setCargandoContraprestaciones(false)
      })
  }
  ...
  const submitAprobarContraprestacion = async () => {
    if (!selectedFile) {
      setFileError(
        'Debe seleccionar un archivo para aprobar la contraprestación'
      )
      return
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (contentType && contentType.indexOf('application/json') !== -1) {
          ...
          throw new Error(
            errorData.mensaje || 'Error al generar el certificado'
          )
        } else {
    ...
    <div className='w-full p-4 relative'>
      <div className='w-full flex items-center justify-between mb-8'>
        <p className='text-center text-titulos flex-1'>
          Lista de Contraprestaciones
        </p>
        <Boton onClick={() => navigate('/contraprestaciones/crear')}>
      ...
      <TablaEstados
        ...
        cargandoContenido={cargandoContraprestaciones}
      />
        ...
        cuerpo={
          <div>
            ...
            {currentContraprestacion && (
              <p>
                ¿Estás seguro que quieres aprobar la contraprestación de{' '}
                {currentContraprestacion.Estudiante} realizada en el semestre{' '}
                {currentContraprestacion.Semestre}?
              </p>
            )}

            <p className='text-normal mt-6 mb-2'>
              Informe de Contraprestación (PDF o DOCX)
            </p>
