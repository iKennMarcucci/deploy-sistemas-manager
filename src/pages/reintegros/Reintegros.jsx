...
const Reintegros = () => {
    ...
    // Estado para la descarga de soporte
    ...
    const fetchSolicitudes = () => {
        fetch(`${backendUrl}/reintegro`)
            ...
            .catch((error) => {
                showAlerta("Error al cargar las solicitudes de reintegro", "error", "Error de conexión");
            });
    };
    ...
    const submitAprobarReintegro = async () => {
        ...
        try {
            const response = await fetch(`${backendUrl}/reintegro/aprobar/${currentSolicitud.Id}`, {
                method: 'POST',
                body: formData
            });
            ...
            setSelectedFile(null);
            setFileName('');
            showAlerta(data.mensaje || 'Solicitud de reintegro aprobada con éxito', 'success', 'Reintegro aprobado');
        } catch (err) {
    ...
    const crearSolicitudReintegro = async (e) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                const errorMessage = data.mensaje || data.message || data.error || 'Error al crear la solicitud de reintegro';
    ...
    const actualizarSolicitudReintegro = async (e) => {
        ...
        try {
            const response = await fetch(`${backendUrl}/reintegro/actualizar/${editSolicitudId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            if (!response.ok) {
                // Si la respuesta no es exitosa, extraemos el mensaje de error
                const errorMessage = data.mensaje || data.message || data.error || 'Error al actualizar la solicitud de reintegro';
                throw new Error(errorMessage);
            }
            ...
            fetchSolicitudes(); // Actualizar tabla de solicitudes
            showAlerta(data.mensaje || 'Solicitud de reintegro actualizada con éxito', 'success', 'Reintegro actualizado');
        } catch (error) {
    ...
    return (
        <div className="w-full p-4">
            <p className="text-center text-titulos">Reintegro de Estudiantes</p>
                ...
                cuerpo={
                    <div>
                        <div className="flex flex-col gap-1 text-center mb-6">
                            <p className="text-2xl font-semibold text-titulos">Aprobar Solicitud de Reintegro</p>
                        </div>

                        {currentSolicitud && (
                            <p>
                                ¿Está seguro que desea aprobar la solicitud de reintegro para el estudiante {currentSolicitud.Estudiante}?
                            </p>
                        )}
                        <p className="mt-2 mb-1">
                            Esta acción aprobará el reintegro para el semestre {currentSolicitud?.["Semestre Reintegro"]}, habilitará al estudiante para matricular materias y cambiará el estado del estudiante a "En curso".
                        </p>
