...
const VerReintegro = () => {
    ...
    const fetchReintegroData = () => {
        setLoading(true);
        fetch(`${backendUrl}/solicitud/${id}`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('No se pudo cargar la información del reintegro');
                }
            ...
            .catch(err => {
                ...
                setLoading(false);
                showAlerta('No se pudo cargar la información del reintegro', 'error', 'Error de conexión');
            });
    };
    ...
    const handleApproveReintegro = async () => {
        if (!selectedFile) {
        ...
        try {
            ...
            showAlerta(data.mensaje || 'Reintegro aprobado con éxito', 'success', 'Solicitud aprobada');
        } catch (err) {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center h-screen">
                ...
                <Boton onClick={() => navigate('/reintegros')} startContent={<ArrowLeft size={18} />}>
                    Volver a Reintegros
                </Boton>
            </div>
        );
    ...
    return (
        <div className='flex flex-col w-full items-center p-4'>
            <div className='w-full flex flex-row justify-between mb-6'>
                ...
                <p className='text-center text-titulos'>Detalles de la Solicitud de Reintegro</p>
                <div className='w-[40px]'></div>
            </div>

            {/* Información del reintegro */}
            <p className='text-normal'>Información del Reintegro</p>
            <Divider className='mb-4' />

            <div className='w-full flex flex-col'>
                {/* Primera fila: Estudiante y Semestre de Aplazamiento */}
                <div className='w-full flex flex-row'>
                    ...
                    <Input
                        ...
                        readOnly
                        className='mb-4'
                        label='Semestre de Aplazamiento'
                        labelPlacement='outside-left'
                        type='text'
                        value={solicitud.semestreAplazamiento || 'No especificado'}
                    />
                </div>

                {/* Segunda fila: Fecha de solicitud y Estado */}
                <div className='w-full flex flex-row'>
                    <Input
                        classNames={{
                            label: `w-1/4 h-[40px] flex items-center group-data-[has-helper=true]:pt-0`,
            ...
            {/* Informe del reintegro (solo se muestra si está aprobado) */}
            {solicitud.estaAprobado && solicitud.soporte && (
                <>
                    <p className='text-normal mt-8'>Informe de Reintegro</p>
                    <Divider className='mb-4' />
                    <div className='w-full px-4'>
            ...
            <Modal
                ...
                footer={
                    <div className="flex justify-end w-full">
                        <Boton
                            color="success"
                            onClick={handleApproveReintegro}
