...
export default function CrearInformeForm({ isOpen, onClose, onSubmit, grupoCohorteId }) {
   ...
   const handleSubmit = async (e) => {
      ...
      setError(null)
      try {
         await onSubmit({ ...form, grupoCohorteId })
         setForm({ descripcion: "", fecha: "", hora: "", lugar: "" })
         onClose()
      } catch (err) {
      ...
      <main className="bg-black/50 backdrop-blur-md flex justify-center items-center fixed w-full h-full z-50 left-0 top-0">
         <section ref={modalRef} className="bg-white modal-animation max-w-xl w-full rounded-md p-6">
            <header className="flex justify-between items-center">
               <h6 className="text-lg font-semibold">Nuevo Informe</h6>
               <button onClick={onClose} className="text-black/50 hover:text-black duration-150">
                  <XIcon size={20} />
               </button>
            </header>

            <p className="text-sm text-black/50">Complete el formulario para crear un nuevo informe</p>
